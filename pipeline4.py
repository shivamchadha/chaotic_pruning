...
data = "cancer" 
datapath = rf"data/{data}.csv"
init = np.load(rf"initializations/{data}npy",allow_pickle=True)
num_epochs = 5
learning_rate = 0.5
mask = False
...
if mask:
    mask_list = (np.genfromtxt(rf"masks/mask_{data}_{initialization}.csv", delimiter=',')).tolist()
    lim0 = 1
    model_tensors = {}
...
prof = torch.profiler.profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler(rf"./log/{data}"),
